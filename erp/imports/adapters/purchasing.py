...
Purchase invoices (FILE_15 Task B) mirror ``sales.SalesInvoiceAdapter`` exactly — see that class's
docstring for the "no distinct invoice document, no user-settable order number" reasoning, which
applies here unchanged (``PurchaseOrder`` has the identical shape). Writes go through
``erp.purchasing.contracts.create_order`` (re-exported from ``services.orders`` — ``place_order``
doesn't expose ``tax_code``).
"""
...
# --- purchase invoices (FILE_15 Task B) ------------------------------------------------------------
