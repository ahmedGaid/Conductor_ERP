...
| crm | Leads (capture→qualify→convert), Opportunities/pipeline (qualifying→proposal→negotiation→won/lost; **won creates a draft sales order via the sales contract**, by customer code), Activities (log/complete), support Tickets with priority-driven **SLA** (breach detection); DRF API + React screens. Planned: campaigns + lead scoring, ticket escalation, account/contact unification | 5 | core done (gate09); rest planned |
