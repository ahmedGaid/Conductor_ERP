...
| B22 | Document/finance adapter review screens — `SI/FILE_15`–`FILE_16` UI half | **A** (cross-lane, founder-authorized 2026-07-23 — FILE_15 also gained a backend-refactor scope expansion (group-aware analyze pass), done by A too, same authorization) | `erp/imports/registry.py`/`engine.py`/analyze-validate path (FILE_15 CONFIRMED SCOPE), `apps/web/src/pages/imports/**` (adapter preview + finance suspense-approval panel) | B11, B12 (done) | 1–2 | M3 | done(A) — FILE_15 half only, see `erp-status`/plan file; FILE_16 finance suspense-approval panel still open for B |
