...
| **8D** | `Docs/plan/post-handover-v1_1/` | FILE_01 → FILE_05 (+P2 06–09) | **"The rest" — QA-audit Phase 1/2 (added 2026-07-18). AFTER handover, interleavable once live.** FILE_01 enforce ruff/mypy/bandit in CI, FILE_02 dep lockfile+Dependabot, FILE_03 backend coverage, FILE_04 frontend Vitest unit tests, FILE_05 README cross-platform + Django6 deprecation fix. Phase 2 (Nice-to-Have): a11y CI, OpenAPI, arch diagrams, load test. **References-not-duplicates** smart-import UI (pos 10), admin panel/AI-cost (pos 9 FILE_19/20), AI reliability (pos 12). Index: `FILE_00_INDEX.md` |
...
| **H** | Dynamic-help Live-checklist rollout (no plan folder — tracked here + commit history) | 12/77 guides done | **Founder-approved, off critical path (2026-07-17).** Roll the Live-tab checklist pattern (alerts + step-by-step hand-holding, see `apps/web/src/help/HelpSignalsContext.tsx` + `HelpCenter.tsx`) to the remaining 65 help guides in `apps/web/src/help/content/*.ts`, batch by batch (~3 pages/batch: read page state → design signals → write guide → wire `useSetHelpSignals` → gates → live-verify → commit). Done so far: Webhooks, New Order, New PO, Journal Entry, Users invite, Leads, Items, New Quotation, Tickets, New Purchase Request, Suppliers, Customers. Candidates next: CRM Campaigns/Pipeline, Price Lists, Inventory Stock Counts/Movements, Settings Branches/Organization. Pause/resume freely between queue positions — does not block pos 8 handover. |
...
| **P** | `Docs/plan/arp-roadmap.md` → "Craft & Trust polish" stubs (no folder yet — scope a `FILE_00` when scheduled) | 0/3 | **Craft & Trust polish (added 2026-07-18, off critical path, founder-standing-OK).** Three on-brand adds folded from the CPO "Master Plan" review: (1) System Confidence panel — a positive health strip complementing the shipped "Needs attention today"; (2) calm milestone moments — no confetti/sound; (3) English product-vocabulary canon — extend the Identity §6 lexicon moat to English nouns. Slots into gaps between numbered positions; does **not** block pos 8 handover. Rejected/guarded items (sounds, silent autosave, column-resize, NL→SQL) are recorded in DECISIONS.md — do not re-queue. |
