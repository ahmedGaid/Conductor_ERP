...
| **7** | `Docs/plan/os-foundations-plan/` | folder created via ag-plan when reached | **Phase W+ remainder** — Action Graph v2, verifier packs, simulation diff card (deep vision L0–L2; adopted 2026-07-07). MUST dedupe on creation: eval harness → pos 6 FILE_01; answer self-verification → ai-reliability FILE_05 T5.8 |
| **8** | `Docs/plan/smart-import-plan/` | FILE_01 → FILE_17 | **Phase A** — Smart Import Engine: zero-prep Excel migration (detect, map, clean, dedupe, auto-masters, validate, import, rollback) |
| **9** | `Docs/plan/arp-roadmap.md` | phases A2, B, B2, C–F | The strategic roadmap — only after 1–8 are fully `_done` |
| **10** | `Docs/plan/ai-reliability-roadmap/` | FILE_03 → FILE_08 | **AI engineering long track** (Phases 3–8: retrieval v2, memory, agent orchestration v2, guardrails/security, perf/cost, production hardening). May interleave with pos 9 arp phases at merge checkpoints — founder paces it |
