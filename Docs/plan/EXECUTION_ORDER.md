...
| **8C** | `Docs/plan/einvoice-eta-live/` | FILE_01 → FILE_06 | **CONFIRMED Must-Have handover blocker (founder picked Branch A, 2026-07-18 — see `DECISIONS.md`).** Real ETA submission, replaces the simulated `eta_adapter.py` stub. Credentials/sandbox → real submission adapter → signing → status reconcile → archiving+acceptance. **STOP-gate: needs real ETA production/sandbox credentials + the customer's tax profile before FILE_01 there can start** — cannot proceed solo. FILE_06 (added 2026-07-22, not STOP-gated, non-blocking) — real product coding (GPC/EGS/GTIN), current `itemCode` is a SKU placeholder. Index: `FILE_00_INDEX.md` |
| **8D** | `Docs/plan/post-handover-v1_1/` | FILE_01 → FILE_06 **done** (+P2 07–09 not started) | **"The rest" — QA-audit Phase 1/2 (added 2026-07-18).** Phase 1 (FILE_01–05) all done 2026-07-19 — ran ahead of the "after handover" note since none is customer-facing (see `FILE_00_INDEX.md` change log + `DECISIONS.md`). FILE_01 ruff/mypy/bandit CI lint job (baselined ~1679 ruff + 202 mypy + 3494 bandit findings), FILE_02 dep lockfile (`pip-compile`) + Dependabot, FILE_03 backend coverage baseline (89%, floor 84%), FILE_04 frontend Vitest unit tests (money/customFields/workflow, 39 passing), FILE_05 README cross-platform + Django6 deprecation fix. **FILE_06 a11y CI (axe-core) done 2026-07-23** — founder picked it from the Phase-2 menu; found+fixed 3 real contrast/label bugs (purchasing/inventory module-accent, `--color-text-muted`, missing line-item `aria-label`s), flagged 1 as a deliberate/mitigated tradeoff (`--color-text-subtle`, opt-in high-contrast already covers it) — detail in `DECISIONS.md`. Remaining Phase 2 (Nice-to-Have, not started): OpenAPI, arch diagrams, load test. **References-not-duplicates** smart-import UI (pos 10), admin panel/AI-cost (pos 9 FILE_19/20), AI reliability (pos 12). Index: `FILE_00_INDEX.md` |
| **LK** | `Docs/plan/license-key-plan/` | FILE_01 → FILE_06 | **Perpetual license key (added 2026-09-24, off critical path).** Sell Conductor as a one-time purchase (unlimited users) + optional annual maintenance: Ed25519-signed key verified offline, bound to the company tax ID, gates edition modules/branch count/AI add-on, and `manage.py upgrade` refuses releases dated after `maintenance_until`. `LICENSE_MODE=off` by default → zero behaviour change for dev/demo/SaaS. Data never locked (read-only floor keeps reads + exports). **Must land before the first perpetual-license sale** — does not block the pos 8 handover unless that customer buys perpetual. FILE_01 = Opus (crypto core), rest Sonnet. Index: `FILE_00_INDEX.md` |
...
| ~~**10**~~ ✅ | `Docs/plan/smart-import-plan/` | FILE_01 → FILE_17 **DONE (acceptance signed off 2026-07-26)** | **Phase A CLOSED** — Smart Import Engine: zero-prep Excel migration (detect, map, clean, dedupe, auto-masters, validate, import, rollback), background runner verified at real 100k-row scale, trial-balance opening correction-approval flow. Detail → `DECISIONS.md` "Smart Import Engine — FILE_17 acceptance sign-off". **Merged to `main` 2026-07-26 (`34000c4`).** |
...
| **H** | Dynamic-help Live-checklist rollout (no plan folder — tracked here + commit history) | 19/77 guides done | **Founder-approved, off critical path (2026-07-17). Standing brand rule since 2026-07-24 (`Conductor_ERP_Product_Design_Engineering_Directive.md`) — target state for EVERY page, not an optional extra.** Roll the Live-tab checklist pattern (alerts + step-by-step hand-holding, see `apps/web/src/help/HelpSignalsContext.tsx` + `HelpCenter.tsx`) to the remaining 58 help guides in `apps/web/src/help/content/*.ts`, batch by batch (~3 pages/batch: read page state → design signals → write guide → wire `useSetHelpSignals` → gates → live-verify → commit). Done so far: Webhooks, New Order, New PO, Journal Entry, Users invite, Leads, Items, New Quotation, Tickets, New Purchase Request, Suppliers, Customers, CRM Campaigns, CRM Pipeline, Price Lists, Inventory Stock Counts, Inventory Movements, Settings Branches, **Settings Organization (2026-07-24 — real tax-profile checklist: country/VAT/e-invoicing, live-verified reacting to real field edits)**. Candidates next: remaining Sales/Purchasing/Accounting detail pages (need alerts, not checklists — a view/action page has nothing to "complete"). Pause/resume freely between queue positions — does not block pos 8 handover. |
...
| **WB** | `Docs/superpowers/plans/2026-07-22-nontechnical-workflow-builder.md` | Task 1 → Task 14 | **Non-technical workflow builder (added 2026-07-22, off critical path, founder-approved design).** SMB owners must build/read automations with no developer in the loop — current canvas+JSON authoring is dev-shaped. 14 TDD tasks: notification node type, `WorkflowTrigger` (event-based, reuses the webhook event catalog) + scheduled triggers (low stock/overdue invoice/stale lead/ticket escalation, mirrors `accounting.run_scheduled_reports`), 5-template registry, template picker + step-list builder UI (no canvas/JSON), canvas demoted to `/workflows/advanced`, Arabic-only walkthrough acceptance. One task = one session (`superpowers:executing-plans` / `subagent-driven-development`) — resume by opening the plan file and finding the first unchecked box. Design spec: `Docs/superpowers/specs/2026-07-22-nontechnical-workflow-builder-design.md`. |
