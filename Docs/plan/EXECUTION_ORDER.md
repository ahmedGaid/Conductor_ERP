...
| **PA** | `Docs/plan/agent-posting-plan/` | FILE_01 → FILE_08 | **Posting actions — Option B reopened, scoped down (added 2026-07-19/20).** Founder decision: proceed WITHOUT waiting for ai-reliability FILE_05 (unbuilt, multi-month prerequisite) — manual guards instead (org-wide `OrgPreferences.assistant_posting_enabled` toggle + reused per-action role checks + typed retype-confirm on `risk="post"` actions). 6 new actions: post a drafted journal entry (new domain gap found — also gets a manual "Post" button, not assistant-only), receive/bill/pay a purchase order, approve a purchase request, issue stock. Off critical path (does not block customer handover), founder-paced, may be pulled to NEXT same as ★ was. Index: `FILE_00_INDEX.md` |
